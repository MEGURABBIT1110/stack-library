...
親の`development_lead`だけが、公式リモートMCP endpoint `https://mcp.microcms.io/mcp/meguru-stack-library`へ直接接続します。bearer tokenはCodex host/user environment variable名`MICROCMS_API_KEY`から供給し、アプリruntime用の`.env.local`だけをMCP credential sourceとして扱いません。値をshell、model context、repository、Issue、ログ、handoffへ出力・記録しません。app runtimeとdevelopment MCPは分離し、ローカルMCP serverは起動せず、live callをsubagentへ委譲しません。親のruntime inventoryは公式server/APIの接続設定に従って実行時に確認し、tool数やallowlistを固定要件にしません。books APIへのcreateは唯一のmutationでprompt approvalを必須とします。exact target、payload digest、expected state、明示的なone-shot user authorizationがない場合は呼び出しません。update、delete、draft、status、reservation、media、Management、member、bulk、unknown toolは禁止です。timeoutまたはunknown response後はretryせず、直接HTTP、CLI、browserへfallbackしません。create後は同じserviceのMCP read-backがexpected stateとMATCHした場合だけ`mutation_verified=VERIFIED`です。
